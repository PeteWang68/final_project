...
slr = lm(crime_rate~share_voters_voted_trump, data = merge_data_log)
summary(slr)
```
...
hate_crime_ave<-hate_crime %>% 
  na.omit() %>% 
  filter(year!=c(2017,2016)) %>%
  group_by(state) %>% 
  summarise(ave_annualprop = (sum(annualprop)/n_distinct(year))) 
...
compare_rate %>% 
  mutate(hate_crimes_per_100k_year = hate_crimes_per_100k_splc/10 * 365) %>% 
  select(state, hate_crimes_per_100k_year, ave_annualprop) %>% 
  melt(, id = "state") %>% 
...
compare_rate %>% 
  ...
  select(state, hate_crimes_10days_year, ave_annualprop) %>% 
  melt(, id = "state") %>% 
